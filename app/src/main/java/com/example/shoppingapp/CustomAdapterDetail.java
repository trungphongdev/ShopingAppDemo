...
public class CustomAdapterDetail extends RecyclerView.Adapter<CustomAdapterDetail.ViewHolder> {
    private List<ModelItemDetail> list;
   private Context context;
    public CustomAdapterDetail(List<ModelItemDetail> list, Context context) {
        this.list = list;
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.custom_item_row_detail,parent,false);
        return new  ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ModelItemDetail item = list.get(position);
        holder.txvTensp.setText(item.name);
        holder.txvGiamgia.setText(item.discount);
        holder.txvGiabandau.setText(item.firstPrice);
